...
static WordNode* MakeNode(char* newWord){
    ...
    // if node was created, set initial values
    if (new_node != NULL)
    {
...
static void InOrderPrintFile(const WordNode* root, FILE* f1) {
    ...
    fprintf(f1, "%7d %s\n", root->count, root->word); //Print the count and word.
    InOrderPrintFile(root->right, f1);
    }
}  
    
    
    /*WordNode* stack[50]; //50 node limit
    int top = -1; //Initialize stack top

    const WordNode* current = root; //Current node

    while (current != NULL || top != -1) { //Loop through tree nodes
        while (current != NULL) { //Go through the leftmost node and moce other nodes along the stack
            stack[top++] = current;
            current = current->left;
        }
        current = stack [top--]; //Find node from stack
        fprintf(f1, "%7d %s\n", current ->count, current->word); //Print the node into this file
        current = current->right; //Move to the right
    }*/ //Possibly could work? It didn't but neither does the other thing. Debugger says this function is the issue.